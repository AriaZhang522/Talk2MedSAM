...
v2_model_path = f"{config.RESULTS_DIR}/fusion_model_bus_best.pth"
...
v4_model_path = f"{config.RESULTS_DIR}/fusion_model_bus_v4_FULL_best.pth"
...
def get_prediction(medsam_model, fusion_model, img_emb, text_embed, bbox, use_fusion=True):
    ...
    sparse, dense = medsam_model.prompt_encoder(
        points=None, boxes=bbox[:, None, :], masks=None
    )
...
def visualize_bus_comparison(medsam_model, fusion_v2, fusion_v4, val_loader, device, 
                             birads_emb, num_samples=5):
\
\
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        print(f"\n   Sample {sample_count + 1}: {image_id} | BI-RADS: {birads}")
        
                   
        text_embed = birads_emb[birads].unsqueeze(0).to(device)
        ...
        print(f"      Dice → Baseline: {dice_baseline:.4f} | Gated: {dice_v2:.4f} | Hierarchical: {dice_v4:.4f}")
        ...
        axes[1].imshow(masked_baseline, cmap='Oranges', alpha=0.5, vmin=0, vmax=1)
        axes[1].contour(mask_baseline, colors='orange', linewidths=2)
        ...
        axes[2].imshow(masked_v2, cmap='Greens', alpha=0.5, vmin=0, vmax=1)
        axes[2].contour(mask_v2, colors='lime', linewidths=2)
        axes[2].set_title(f'Gated (V2)\nDice: {dice_v2:.4f}', fontsize=12, fontweight='bold', color='darkgreen')
        axes[2].axis('off')
        
                                  
        axes[3].imshow(img_np)
        masked_v4 = np.ma.masked_where(mask_v4 == 0, mask_v4)
        axes[3].imshow(masked_v4, cmap='Blues', alpha=0.5, vmin=0, vmax=1)
        axes[3].contour(mask_v4, colors='blue', linewidths=2)
        axes[3].set_title(f'Hierarchical (V4)\nDice: {dice_v4:.4f}', fontsize=12, fontweight='bold', color='darkblue')
        axes[3].axis('off')
        ...
        save_path = f"bus_visualization_sample_{sample_count + 1}.png"
